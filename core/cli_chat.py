...
class CliChat(Chat):
    ...
    async def list_docs_ids(self) -> list[str]:
        """
        Devuelve la lista de IDs de documentos desde el servidor MCP.

        Usa el recurso 'docs://all', que hemos definido en mcp_server.py
        para listar todas las claves del diccionario docs.
        """
        result = await self.doc_client.read_resource("docs://documents")

        # Si el servidor devuelve directamente una lista, úsala tal cual
        if isinstance(result, list):
            return [str(x) for x in result]

        # Fallback por si en algún momento devolvemos otro tipo de estructura
        return [str(result)]
    ...
    async def _extract_resources(self, query: str) -> str:
        # Extraer menciones que empiezan con @ y limpiar puntuación final
        raw_words = query.split()
        mentions: list[str] = []

        for word in raw_words:
            if word.startswith("@"):
                # quitamos la @ del principio
                doc = word[1:]
                # quitamos ?,.,,;:! etc del FINAL (no del medio, así conserva .pdf)
                doc = doc.rstrip(string.punctuation)
                if doc:
                    mentions.append(doc)

        # Pedimos al servidor MCP la lista de IDs válidos
        ...
        # Construimos el bloque <document>...</document> que va al prompt
        return "".join(
            f'\n<document id="{doc_id}">\n{content}\n</document>\n'
